...
class BracuChickenGame extends FlameGame with PanDetector, HasCollisionDetection {
  ...
  Future<void> onLoad() async {
    ...
    if (background == 'forest1') {
      parallax!.y -= 12; //(adjust as needed)
    }
    add(parallax!);
  ...
  void onPanEnd(DragEndInfo info) {
    if (_swipeStart != null && _swipeEnd != null) {
      ...
      print('Swipe delta: dx=[32m[1m[4m${delta.x}[0m, dy=[32m[1m[4m${delta.y}[0m, length=${delta.length}');
  ...
  Future<void> reloadParallax() async {
    ...
    // Apply a y offset if forest1 is selected
    if (background == 'forest1') {
      parallax!.y -= 40; // Move the ground up by 40 pixels (adjust as needed)
    }
    add(parallax!);
...
class Obstacle extends PositionComponent with HasGameRef<BracuChickenGame>, CollisionCallbacks {
  double speed;
